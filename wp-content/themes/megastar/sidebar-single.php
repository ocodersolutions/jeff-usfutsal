<?php 
	...
	if(is_page() || is_page_template('page-blog.php')){
		// Page Sidebar 
		if ( !function_exists( 'dynamic_sidebar' ) || !dynamic_sidebar(get_post_meta( get_the_ID(), 'megastar_sidebar', true )) );
	} elseif(is_search()){
		// Search Results Sidebar 
		if ( !function_exists( 'dynamic_sidebar' ) || !dynamic_sidebar('Search Results Widgets') );
	} else {
		// Blog Sidebar && Default Sidebar
		if ( !function_exists( 'dynamic_sidebar' ) || !dynamic_sidebar('widget_single_page') );
	}
