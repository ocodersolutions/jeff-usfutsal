jQuery(document).ready(function ($) {
    ...
    $("#compile_less").on('click', function (e) {
        ...
        $.when($.ajax({
            url: ajaxurl,
            type: "post",
            data: {action: "megastar_get_styles"},
            ...
            if (jqXHR.status == 200) {
                ...
                $.ajax({
                    url: ajaxurl,
                    type: "post",
                    data: {action: "megastar_save_styles", styles: JSON.stringify(s)},
    ...
    var saveInterval;
    var savedText = $('#save').val();
    $('#customize-controls #save').on('click', function () {
        var $that = $(this);
        if (saveInterval)
            clearInterval(saveInterval);
        saveInterval = setInterval(function () {
            if ($that.val() == savedText) {
                $that.val(adminSettingMessage.compilingLess)
                $.when($.ajax({
                    url: ajaxurl,
                    type: "post",
                    data: {action: "megastar_get_styles"},
                    ...
                    if (jqXHR.status == 200) {
                        ...
                        $.ajax({
                            ...
                            success: function (data) {
                                var result_message = '';
                                for (var message in data) {
                                    result_message += data[message].message.replace(/(<([^>]+)>)/ig, "") + "\r\n";
                                }
                                try {
                            ...
                            error: function (xhr, ajaxOptions, thrownError) {
                                try {
                                    console.log(adminSettingMessage.compilingLessError);
                                } catch (err) {
