...
class megastar_nav_dom {
    ...
    **/
    ...
    $count_li = count($ul->children()); 
        ...
        $li->append('<div class="megamenu"><div class="megamenu-row"></div></div>');
        $div = $li->first('div.megamenu div.megamenu-row');
         foreach ($li->children('ul') as $i => $u) {
            $u->removeClass('dropdown');
 ...
 */
class megastar_dom_document extends DOMDocument {
    ...
    public function __construct($version = '1.0', $encoding = 'UTF-8') {
        ...
        // set node class
        $this->registerNodeClass('DOMElement', 'megastar_dom_element');
    }
    ...
    public function find($query) {
        return $this->xpath()->query(megastar_dom_css_selector::toXPath($query, 'descendant::'));
    }
 ...
 */
class megastar_dom_element extends DOMElement {
    ...
    public function find($query) {
        return $this->query(megastar_dom_css_selector::toXPath($query, 'descendant::'));
    }
...
public function children($query = null) {
    ...
    return $this->query(megastar_dom_css_selector::toXPath($query, 'child::'));
}
...
protected function prepareInsert($item) {
    ...
    if (is_string($item)) {
        ...
        $item = megastar_dom_entities::replaceAllEntities($item);
 ...
 */
class megastar_dom_entities {

protected static $regex = '/&([\w]+);|&#([\d]+);|&#(x[0-9a-fA-F]+);|(&)/m';

public static function replaceAllEntities($string) {
    return preg_replace_callback(self::$regex, 'megastar_dom_entities::doReplacement', $string);
}
 ...
 */
class megastar_dom_css_selector {
