...
add_filter( 'wp_setup_nav_menu_item', 'megastar_add_custom_nav_fields' );

function megastar_add_custom_nav_fields( $menu_item ) {
...
add_action( 'wp_update_nav_menu_item', 'megastar_update_custom_nav_fields', 10, 3 );

function megastar_update_custom_nav_fields( $menu_id, $menu_item_db_id, $args ) {
...
// edit menu walker
add_filter( 'wp_edit_nav_menu_walker', 'megastar_menu_edit_walker', 10, 2 );

function megastar_menu_edit_walker($walker = '', $menu_id = '') {
    $menu_locations = get_nav_menu_locations();
 
    if($menu_id != $menu_locations['primary'])  return 'Walker_Nav_Menu_Edit';
    return 'megastar_Walker_Nav_Menu_Edit';
}

// Create HTML list of nav menu input items.
// Extend from Walker_Nav_Menu class
class megastar_Walker_Nav_Menu_Edit extends Walker_Nav_Menu  {
    /**
    ...
    <p class="field-link-target description">
        <label for="edit-menu-item-target-<?php echo esc_attr($item_id); ?>">
            ...
            <?php echo esc_html_x( 'Open link in a new window/tab', 'backend', 'megastar' ); ?>
        </label>
    </p>
    <p class="field-css-classes description description-thin">
        <label for="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>">
            <?php echo esc_html_x( 'CSS Classes (optional)', 'backend', 'megastar' ); ?><br />
            <input type="text" id="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-classes" name="menu-item-classes[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( implode(' ', $item->classes ) ); ?>" />
        </label>
    </p>
    <p class="field-xfn description description-thin">
        <label for="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>">
            <?php echo esc_html_x( 'Link Relationship (XFN)', 'backend', 'megastar' ); ?><br />
    ...
    ?>
    <p class="description description-wide megastar-menu-setting-icon">
        <label for="edit-menu-item-icon-<?php echo esc_attr($item_id); ?>">
            ...
            <input type="text" id="edit-menu-item-icon-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-icon"
                   ...
                   value="<?php echo esc_attr( $item->icon ); ?>" placeholder="uk-icon-home" />
            <span><?php printf(esc_html_x('Input uikit icon class to show this menu icon. You can see %s here. For example: uk-icon-home', 'backend', 'megastar' ), '<a target="_blank" href="http://getuikit.com/docs/icon.html#icon-mapping">Uikt Icon</a>') ?></span>
        </label>
    </p>
   
    <p class="description description-thin megastar-menu-setting-columns">
        <label for="edit-menu-item-columns-<?php echo esc_attr($item_id); ?>">
            <?php echo esc_html_x( 'Dropdown Columns', 'backend', 'megastar' ); ?><br />
            <input type="text" id="edit-menu-item-columns<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-columns"
                    name="menu-item-columns[<?php echo esc_attr($item_id); ?>]"
                   data-name="menu-item-columns[<?php echo esc_attr($item_id); ?>]"
                   value="<?php echo esc_attr( $item->columns ); ?>" />
        </label>
    </p>
    <p class="description description-thin megastar-menu-setting-fullwidth">
        <label for="edit-menu-item-full_width-<?php echo esc_attr($item_id); ?>">
            <?php echo esc_html_x( 'Dropdown Fullwidth', 'backend', 'megastar' ); ?><br />             
        <select id="edit-menu-item-full_width<?php echo esc_attr($item_id); ?>"
                    ...
                    class="widefat">
                    <option value="" <?php if(!esc_attr($item->full_width)){echo 'selected="selected"';} ?>>No</option>
                    <option value="1" <?php if(esc_attr($item->full_width)){echo 'selected="selected"';} ?>>Yes</option>
                </select>
        </label>
    </p>
    <p class="description description-thin megastar-menu-setting-alignment">
        <label for="edit-menu-item-style_position-<?php echo esc_attr($item_id); ?>">
            <?php echo esc_html_x( 'Dropdown Menu Alignment', 'backend', 'megastar' ); ?><br />             
    ...
    <br/>
    <div class="description description-wide megastar-menu-setting-background">
        <label for="edit-menu-item-dropdown_background-<?php echo esc_attr($item_id); ?>">
            <?php echo esc_html_x( 'Dropdown Menu Background', 'backend', 'megastar' ); ?><br />  
        </label>
        ...
        <div class="attachment-media-view">
            <div class="placeholder" id="image_preview_<?php echo esc_attr($item_id); ?>">
                <?php if (isset($item->dropdown_background) && $item->dropdown_background != ''): ?>
                    ...
                    <img  style="max-width: 388px" src="<?php echo $item->dropdown_background ?>" />
                <?php else : ?>
                <?php echo esc_html_x('No image selected', 'backend', 'megastar'); ?>
                <?php endif; ?> 
            </div>
            <div class="actions">
                <input type="hidden" name="menu-item-dropdown_background[<?php echo esc_attr($item_id); ?>]" value="<?php echo $item->dropdown_background ? $item->dropdown_background : '' ?>" id="image_link_<?php echo esc_attr($item_id); ?>" />
    ...
    <p class="description description-thin megastar-menu-setting-dropdown-classic">
        <label for="edit-menu-item-dropdown_child-<?php echo esc_attr($item_id); ?>">
            <?php echo esc_html_x( 'Dropdown Classic', 'backend', 'megastar' ); ?><br />             
    ...
    <p class="field-move hide-if-no-js description description-wide">
        <label>
            <span><?php echo esc_html_x( 'Move', 'backend', 'megastar' ); ?></span>
            <a href="#" class="menus-move menus-move-up" data-dir="up"><?php echo esc_html_x( 'Up one', 'backend', 'megastar' ); ?></a>
            <a href="#" class="menus-move menus-move-down" data-dir="down"><?php echo esc_html_x( 'Down one', 'backend', 'megastar' ); ?></a>
            <a href="#" class="menus-move menus-move-left" data-dir="left"></a>
            <a href="#" class="menus-move menus-move-right" data-dir="right"></a>
            <a href="#" class="menus-move menus-move-top" data-dir="top"><?php echo esc_html_x( 'To the top', 'backend', 'megastar' ); ?></a>
        </label>
    </p>
