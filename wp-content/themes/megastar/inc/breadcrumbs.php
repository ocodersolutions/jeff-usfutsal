...
function megastar_breadcrumbs() {
    ...
    if (is_home() || is_front_page()) {
        ...
        if (is_category()) {
            ...
            $output[] = $before . esc_html__('Category', 'megastar') . ': ' . esc_html(single_cat_title('', false)) . '' . $after;
            ...
            $output[] = $before . esc_html__('Tag', 'megastar') . ': ' . esc_html(single_tag_title('', false)) . $after;
        } elseif ( is_author() ) {
            global $author;
            $userdata = get_userdata($author);
            $output[] = $before . esc_html__('Articles by', 'megastar') . ' ' . esc_html($userdata->display_name) . $after;
        } elseif ( is_404() ) {
            $output[] = $before . esc_html__('Error 404', 'megastar') . $after;
        }
