...
$megastar_woocommerce_limit        = get_theme_mod('megastar_woocommerce_limit', 12);
$megastar_woocommerce_sort         = get_theme_mod('megastar_woocommerce_sort', 1);
$megastar_woocommerce_result_count = get_theme_mod('megastar_woocommerce_result_count', 1);
$megastar_woocommerce_upsells      = get_theme_mod('megastar_woocommerce_upsells', 0);
$megastar_woocommerce_related      = get_theme_mod('megastar_woocommerce_related', 1);
$megastar_woocommerce_cart_button  = get_theme_mod('megastar_woocommerce_cart_button', 1);
...
add_filter( 'loop_shop_per_page', create_function( '$cols', 'return ' . $megastar_woocommerce_limit . ';' ), 20 );

// Toggle Sort by Function
if($megastar_woocommerce_sort == 0){
	remove_action( 'woocommerce_before_shop_loop', 'woocommerce_catalog_ordering', 30 );
}

// Toggle Result Count
if($megastar_woocommerce_result_count == 0){
	remove_action( 'woocommerce_before_shop_loop', 'woocommerce_result_count', 20 );
}

// Toggle Upsell Products
if($megastar_woocommerce_upsells == 0){
	remove_action( 'woocommerce_after_single_product_summary', 'woocommerce_upsell_display', 15);
}

// Toggle Related Products
if($megastar_woocommerce_related == 0){
	remove_action( 'woocommerce_after_single_product_summary', 'woocommerce_output_related_products', 20);
}

// Toggle Add to Cart Button
if($megastar_woocommerce_cart_button == 0){
...
add_action( 'wp_enqueue_scripts', 'megastar_woo_remove_styles', 99 );
function megastar_woo_remove_styles() {
...
// add_action( 'init', 'megastar_remove_woo_breadcrumbs' );
// function megastar_remove_woo_breadcrumbs() {
...
function megastar_woocommerce_remove_lightbox() {
  // Styles
  wp_dequeue_style( 'woocommerce_prettyPhoto_css' );
  // Scripts
...
add_action( 'wp_enqueue_scripts', 'megastar_woocommerce_remove_lightbox', 99 );

function megastar_woocommerce_splr($html) {
    $html = str_replace('data-rel="prettyPhoto[product-gallery]', 'data-uk-lightbox="{group: \'product-gallery\'}', $html);
    return $html;
}
add_filter('woocommerce_single_product_image_html', 'megastar_woocommerce_splr', 99, 1); // single image
add_filter('woocommerce_single_product_image_thumbnail_html', 'megastar_woocommerce_splr', 99, 1); // thumbnails


/*
 * Predefine woocommerce image size
 */
add_action( 'init', 'megastar_woocommerce_image_dimensions', 1 );
function megastar_woocommerce_image_dimensions() {
  	$catalog = array(
		'width' 	=> '246',	// px
		'height'	=> '316',	// px
		'crop'		=> 1 		// true
