...
if ( class_exists( 'Tribe__Events__Main' ) ) {
	...
	function tribe_get_events_title( $depth = true ) {
		...
		$title = sprintf( esc_html__( 'Upcoming %s', 'megastar'), $events_label_plural );
		...
		if ( isset( $_REQUEST['tribe-bar-date'] ) && $wp_query->have_posts() ) {
			...
			$title = sprintf( esc_html__( '%1$s for %2$s - %3$s', 'megastar'), $events_label_plural, $first_event_date, $last_event_date );
		} elseif ( tribe_is_past() ) {
			$title = sprintf( esc_html__( 'Past %s', 'megastar'), $events_label_plural );
		}

		if ( tribe_is_month() ) {
			$title = sprintf(
				esc_html__( '%1$s for %2$s', 'megastar'),
				$events_label_plural,
	...
	function tribe_events_list_the_date_headers() {
		...
		if ( $show_headers ) {
			...
			if ( $wp_query->current_post === 0 || ( $prev_event_month != $event_month || ( $prev_event_month == $event_month && $prev_event_year != $event_year ) ) ) {
				$html .= sprintf( "<div class='events-list-separator-month-wrapper'><span class='tribe-events-list-separator-month'><span>%s</span></span></div>", tribe_get_start_date( $post, false, $month_year_format ) );
			}
